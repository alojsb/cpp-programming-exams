...
public:
	...
	Poglavlje& operator=(const Poglavlje& desni) {
		if (this != &desni)
		{
			delete[]_naslov;
			delete[]_sadrzaj;
			_naslov = AlocirajNizKaraktera(desni._naslov);
			_sadrzaj = AlocirajNizKaraktera(desni._sadrzaj);
			_prihvaceno = desni._prihvaceno;
	...
	void SetSadrzaj(string noviSadrzaj) {
...
public:
	...
	void SetKonacnaOcjena(float konacnaOcjena) { _konacnaOcjena = konacnaOcjena; }
	...
	void DodajPoglavlje(const char* nazivPoglavlja, const char* sadrzajPoglavlja) {
		...
		for (size_t i = 0; i < _poglavljaRada.size(); i++)
		{
			if (strcmp(_poglavljaRada[i].GetNaslov(), nazivPoglavlja) == 0) {
				_poglavljaRada[i].SetSadrzaj(sadrzajPoglavlja);
				return;
			}
		}

		Poglavlje p(nazivPoglavlja, sadrzajPoglavlja);
...
//parametri:nazivPoglavlja, ocjena
	void OcijeniPoglavlje(const char* nazivPoglavlja, int ocjena) {
		...
		for (size_t i = 0; i < _poglavljaRada.size(); i++)
		{
			if (_poglavljaRada[i].GetNaslov() == nazivPoglavlja) {
	...
	ZavrsniRad& operator=(const ZavrsniRad& desni) {
		if (this != &desni)
		{
			_tema = AlocirajNizKaraktera(desni._tema);
			_poglavljaRada = desni._poglavljaRada;
			_datumOdbrane = desni._datumOdbrane;
			_konacnaOcjena = desni._konacnaOcjena;
		}
		return *this;
	}
...
public:
	...
	void Info() {
		cout << ">> NASTAVNIK <<" << endl;
		cout << "Ime i prezime -> " << GetImePrezime() << endl;
		cout << "Mentorstva:" << endl;
		for (size_t i = 0; i < _teme.getTrenutno(); i++)
		{
			cout << "Student sa br.indeksa " << _teme.getElement1(i) << ", tema zavrsnog rada studenta '" << _teme.getElement2(i).GetNazivTeme() << endl;
		}
	}
...
//parametri: brojIndeksa, zavrsniRad
	bool DodajZavrsniRad(string brojIndeksa, ZavrsniRad zavrsniRad) {
		...
		for (size_t i = 0; i < _teme.getTrenutno(); i++)
		{
			if (strcmp(_teme.getElement2(i).GetNazivTeme(), zavrsniRad.GetNazivTeme()) == 0)
				return false;
			if (strcmp((brojIndeksa).c_str(), (_teme.getElement1(i)).c_str()) == 0 && _teme.getElement2(i).GetNazivTeme() != nullptr)
			{
				return false;
			}
		}
...
//parametri: brojIndeksa, datumOdbrane
	ZavrsniRad* ZakaziOdbranuRada(string brojIndeksa, string datumOdbrane) {
		...
		for (size_t i = 0; i < _teme.getTrenutno(); i++)
		{
			if (strcmp(_teme.getElement1(i).c_str(), brojIndeksa.c_str()) == 0) {
...
string PosaljiPozivZaDodjeluNagrada(Nastavnik** nastavnici, int max, float ocjena) {
	...
	for (size_t i = 0; i < max; i++)
	{
		...
		{
			if (nastavnici[i]->GetTeme().getElement2(j).GetKonacnaOcjena() > 5) {
				trenutniProsjek += nastavnici[i]->GetTeme().getElement2(j).GetKonacnaOcjena();
				brojac++;
			}
		}
		if (brojac != 0) {
